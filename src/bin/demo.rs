...
use messtar::{handshake::Handshake, identity::Identity, session::Session};
...
fn main() {
    ...
    Identity::verify_public_key(&alice_id.verifying_key, alice_pub.as_bytes(), &alice_sig)
        .expect("❌ Alice signature is invalid");

    Identity::verify_public_key(&bob_id.verifying_key, bob_pub.as_bytes(), &bob_sig)
